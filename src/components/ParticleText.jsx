...
// React Bits / ParticleText-JS-CSS. Local adaptations: padded drawing area,
// visible fallback, and suspension outside the viewport / with reduced motion.
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    let visible = true;
    let disposed = false;
    const drawPadding = Math.max(0, overflowPadding);
    ...
    const render = now => {
      ...
      ctx.clearRect(-drawPadding, -drawPadding, width + drawPadding * 2, height + drawPadding * 2);
    ...
    const ensureRenderLoop = () => {
      if (!disposed && visible && !document.hidden && animationFrame === null) {
    ...
    const sampleText = async () => {
      ...
      canvas.width = Math.max(1, Math.floor((width + drawPadding * 2) * dpr));
      canvas.height = Math.max(1, Math.floor((height + drawPadding * 2) * dpr));
      canvas.style.width = `${width + drawPadding * 2}px`;
      canvas.style.height = `${height + drawPadding * 2}px`;
      canvas.style.left = `${-drawPadding}px`;
      canvas.style.top = `${-drawPadding}px`;
      ctx.setTransform(dpr, 0, 0, dpr, drawPadding * dpr, drawPadding * dpr);
    ...
    const handleVisibility = () => {
      if (document.hidden || !visible) stopRenderLoop();
