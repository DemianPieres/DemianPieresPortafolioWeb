...
test("portada de alta calidad, hover reversible y cleanup de movimiento reducido", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto("/");
  const cover = page.locator(".project-cover--image");
  ...
  await page.screenshot({ path: "artifacts/screenshots/project-cover-hover.png" });
  ...
  await cover.scrollIntoViewIfNeeded();
  await page.screenshot({ path: "artifacts/screenshots/project-cover-dark.png" });
});

test("la portada permite scroll táctil sin inclinación", async ({ browser }) => {
  ...
  await cover.screenshot({ path: "artifacts/screenshots/project-cover-mobile.png" });
  await context.close();
});
